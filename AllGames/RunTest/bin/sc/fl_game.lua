...
function 更新钻石显示()
	fl_game:setvali("当前钻石", readi("mojing", 0));
end
function 钻石不足()
	显示游戏结束框();
	...
	for i=0,(game_mjcount_w-1) do
		for j=0,(game_mjcount_h-1) do
			...
			if mjindex >= 0 then
				...
				timeout_1(index*50, "麻将渐显出来", mjindex);
				index = index + 1;
			end
		end
	end
...
function 出现钻石(posin)
	...
	interval_1(50, "慢慢放到钻石框", sindex, 1000);
...
function 慢慢放到钻石框(sindex, tm)
	...
	if tm > 0.35 then
		...
		breaktime();
		interval_1(50, "移动钻石到下方", sindex, 2000);
	end
end

function 移动钻石到下方(sindex, tm)
	local index = atoi(sindex);
...
function 继续游戏()
	播放音效("click");
	interval(1000,"时间减少即时", game_time*2*1000);
...
function ResetGame()
	...
	for imj=0,(tempcnt-1) do
			...
			if bExistMission==false or tempmj[x][y]~=0 then
				...
				timeout_1(cnt*maxtime, "麻将渐显出来", mjindex);
				cnt = cnt + 1;
			else
...
function 麻将渐显出来(idx)
	...
	interval_1(50, "麻将持续渐显出来", idx, 200);
end
lastsel_mj = nil;
...
function 创建消除效果(pos, mjindex, wait)
	...
	interval_1(50, "播放消除特效", index, 10000);
...
function 按照路径播放粒子等待(sindex)
	...
	interval_1(50, "按照路径播放粒子计算", sindex, 10000);
end
	...
	if index1 >= 0 and index2 >= 0 and index1 ~= index2 and int(index1/majiangs_zubie) == int(index2/majiangs_zubie) then--如果两个是同一个麻将
		if 是否可消除(index1, index2, true)==true then
			...
			if current < 10000 then
				mj_chanchu = readi("mj_chanchu"..current, 0);--魔晶出现最大概率-出现数量*(魔晶出现最大概率*0.5);
			end
...
function 计算连击()
	...
	if tm < g_lianji_time then
		...
		if g_combocount > 1 then
			...
			播放音效("beiwin"..(si-1));
			interval(50, "持续计算连击效果", (g_lianji_time+1) * 1000);
	...
	if mouse_down_mj ~= nil then
		if mouse_dir_x ~=0 or mouse_dir_y ~=0 then
			...
			for mj=0,(mouse_down_mjcount-1) do
				...
				interval_1(50, "自动归位", mjindex, 500);
				重新计算 = false;
			end
			g_mousedisable = true;
			timeout(100, "移动完棋子自动计算");
		else
		end
		--
	end
