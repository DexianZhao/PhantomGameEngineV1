...
VOID		RunDocumentProj(const char* proj, unsigned int* guid, const char* szMainLua, int ori)
{
	ActiveGameMode(TRUE);
	SetMyLoadFile(0);//本地运行不需要从网络下载任何数据
	SetPhantomGUID((unsigned int*)guid);
    SetViewOrientation(ori);
	char szNewDoc[512];
...
//		g_manager.SetGameSize(m_gameWidth, m_gameHeight);
        ...
        char currOri[2048];
        currProj[0] = 0;
        loadConfig("current", currProj);
        loadConfig("currentauth", currAuth);
        loadConfig("currentlua", currLua);
        loadConfig("currentori", currOri);
        if(currProj[0]){
            std::vector<unsigned char> guid;
            HexEncode(guid, currAuth);
            if(guid.size() == 512)
                RunDocumentProj(currProj,(unsigned int*)&guid.at(0),currLua, atoi(currOri));
        }
	...
	VOID			OnBeforeDeviceInit()
	{
		...
		strcat(wdrive, "documents");//current);
		strcat(wdrive, "\\");
		g_documents	=	wdrive;
		SetDocumentDir(g_documents.t);
		//
		...
		m_mystate->initState("manager", (unsigned int*)stateGUID);
...
extern "C"{
    VOID StartServer()
    {
        if(!g_bServerRunning){
            ...
            //g_gameMgr.m_service.StartServer();
        }
    }
    VOID StopServer()
    {
        if(g_bServerRunning){
            ...
            //g_gameMgr.m_service.m_server.StopServer();
            //SetLogPrint(0);
            //LogInfo("stop server");
        }
    }
...
INT			GetBrowserUrlCount()
{
	...
	for(int i=0;i<50;i++)
	{
		...
		if(g_gameMgr.m_mystate->find(buf)>=0)
			index++;
	}
	if(index == 0)
	{
		index = 1;
		g_gameMgr.m_mystate->write("browser_0", "http://www.aixspace.com");
	}
	return index;
}
const char* GetBrowserUrl(int index)
{
	...
	return g_gameMgr.m_mystate->readt(buf);
}
namespace Phantom{
...
BOOL		AddBrowserUrl(const char* url)
{
	...
	for(int i=0;i<50;i++)
	{
		char buf[128];
		sprintf(buf, "browser_%d", i);
		const char* szUrl = g_gameMgr.m_mystate->readt(buf);
	...
	char buf[128];
	sprintf(buf, "browser_%d", index);
	g_gameMgr.m_mystate->write(buf, url);
...
VOID DeviceLogPrint(const char* szText)//输出信息
{
	if(g_gameMgr.m_service.m_projectID.id > 0){
		char text[1001];
		memset(text, 0, sizeof(text));
		strncpy(text, szText, 1000);
		g_gameMgr.m_service.m_server.Send(g_gameMgr.m_service.m_projectID, MsgID_LogMessage, text, strlen(text) + 1);
	}
}

GameState*	GetManagerState()
{
	return g_gameMgr.m_mystate;
}
#ifdef ANDROID_OS
