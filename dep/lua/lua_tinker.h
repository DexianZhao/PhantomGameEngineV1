...
namespace lua_tinker
{
	...
	extern	BOOL	g_luaTnikerCallError;
	// call
	template<typename RVal>
	RVal call(lua_State* L, const char* name)
	{
		...
		{
			g_luaTnikerCallError = true;
			print_error(L, "call() attempt to call global `%s' (not a function)", name);
		}
	...
	template<typename T, typename V>
	struct property_var : var_base
	{
		typedef V (T::*gfunc)();
		typedef void (T::*sfunc)(V& v);
		gfunc _gfunc;
		sfunc _sfunc;
		property_var(gfunc gf, sfunc sf) : _gfunc(gf),_sfunc(sf) {}
		void get(lua_State *L)	{ push<V>(L, (read<T*>(L,1)->*_gfunc)());	}//push<typename if_<is_obj<V>::value,V,V>::type>(L, (read<T*>(L,1)->*_gfunc)());	}
		void set(lua_State *L)	{ (read<T*>(L,1)->*_sfunc)(read<V&>(L, 3));	}
	};
	template<typename T, typename V>
	struct property_var_const : var_base
	{
		typedef V (T::*gfunc)();
		typedef void (T::*sfunc)(const V& v);
		gfunc _gfunc;
		sfunc _sfunc;
		property_var_const(gfunc gf, sfunc sf) : _gfunc(gf),_sfunc(sf) {}
		void get(lua_State *L)	{ push<typename if_<is_obj<V>::value,V,V>::type>(L, (read<T*>(L,1)->*_gfunc)());	}
		void set(lua_State *L)	{ (read<T*>(L,1)->*_sfunc)(read<V&>(L, 3));	}
	};
	template<typename T, typename BASE, typename VAR>
	void class_property(lua_State* L, const char* name, VAR (BASE::*gfunc)(), void (BASE::*sfunc)(VAR& v)) 
	{ 
		push_meta(L, class_name<T>::name());
		if(lua_istable(L, -1))
		{
			lua_pushstring(L, name);
			new(lua_newuserdata(L,sizeof(property_var<BASE,VAR>))) property_var<BASE,VAR>(gfunc, sfunc);
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	}
	template<typename T, typename BASE, typename VAR>
	void class_property(lua_State* L, const char* name, VAR (BASE::*gfunc)(), void (BASE::*sfunc)(const VAR& v)) 
	{ 
		push_meta(L, class_name<T>::name());
		if(lua_istable(L, -1))
		{
			lua_pushstring(L, name);
			new(lua_newuserdata(L,sizeof(property_var_const<BASE,VAR>))) property_var_const<BASE,VAR>(gfunc, sfunc);
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	}
	template<typename T, typename V>
	struct property_var2 : var_base
	{
		typedef V (T::*gfunc)();
		typedef void (T::*sfunc)(V v);
		gfunc _gfunc;
		sfunc _sfunc;
		property_var2(gfunc gf, sfunc sf) : _gfunc(gf),_sfunc(sf) {}
		void get(lua_State *L)	{ push<typename if_<is_obj<V>::value,V,V>::type>(L, (read<T*>(L,1)->*_gfunc)());	}
		void set(lua_State *L)	{ (read<T*>(L,1)->*_sfunc)(read<V>(L, 3));	}
	};
	template<typename T, typename BASE, typename VAR>
	...
	{ 
		...
		{
			lua_pushstring(L, name);
			new(lua_newuserdata(L,sizeof(property_var2<BASE,VAR>))) property_var2<BASE,VAR>(gfunc, sfunc);
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	}
	template<typename T, typename V>
	struct property_var3 : var_base
	{
		typedef V (T::*gfunc)(int id);
		typedef void (T::*sfunc)(V v, int id);
		gfunc _gfunc;
		sfunc _sfunc;
		int	  _id;
		property_var3(gfunc gf, sfunc sf, int id) : _gfunc(gf),_sfunc(sf),_id(id) {}
		void get(lua_State *L)	{ push<typename if_<is_obj<V>::value,V,V>::type>(L, (read<T*>(L,1)->*_gfunc)(_id));	}
		void set(lua_State *L)	{ (read<T*>(L,1)->*_sfunc)(read<V>(L, 3),_id);	}
	};
	template<typename T, typename BASE, typename VAR>
	void class_property(lua_State* L, const char* name, VAR (BASE::*gfunc)(int id), void (BASE::*sfunc)(VAR v, int id), int id) 
	{ 
		push_meta(L, class_name<T>::name());
		if(lua_istable(L, -1))
		{
			lua_pushstring(L, name);
			new(lua_newuserdata(L,sizeof(property_var3<BASE,VAR>))) property_var3<BASE,VAR>(gfunc, sfunc, id);
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	}
	template<typename T, typename V>
	struct property_var_ptr : var_base
	{
		typedef V (T::*gfunc)(int id);
		typedef void (T::*sfunc)(const V& v, int id);
		gfunc _gfunc;
		sfunc _sfunc;
		int	  _id;
		property_var_ptr(gfunc gf, sfunc sf, int id) : _gfunc(gf),_sfunc(sf),_id(id) {}
		void get(lua_State *L)	{ push<typename if_<is_obj<V>::value,V,V>::type>(L, (read<T*>(L,1)->*_gfunc)(_id));	}
		void set(lua_State *L)	{ (read<T*>(L,1)->*_sfunc)(read<V>(L, 3),_id);	}
	};
	template<typename T, typename BASE, typename VAR>
	void class_property_ptr(lua_State* L, const char* name, VAR (BASE::*gfunc)(int id), void (BASE::*sfunc)(const VAR& v, int id), int id) 
	{ 
		push_meta(L, class_name<T>::name());
		if(lua_istable(L, -1))
		{
			lua_pushstring(L, name);
			new(lua_newuserdata(L,sizeof(property_var_ptr<BASE,VAR>))) property_var_ptr<BASE,VAR>(gfunc, sfunc, id);
			lua_rawset(L, -3);
		}
		lua_pop(L, 1);
	}
