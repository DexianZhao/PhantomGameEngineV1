...
//	struct p_var : lua_tinker::var_base
//	{
//		typedef V (T::*gfunc)();
//		typedef void (T::*sfunc)(V& v);
//		gfunc _gfunc;
//		sfunc _sfunc;
//		p_var(gfunc gf, sfunc sf) : _gfunc(gf),_sfunc(sf) {}
//		void get(lua_State *L)	{ lua_tinker::push<V>(L, (lua_tinker::read<T*>(L,1)->*_gfunc)());	}//push<typename if_<is_obj<V>::value,V,V>::type>(L, (read<T*>(L,1)->*_gfunc)());	}
//		void set(lua_State *L)	{ (lua_tinker::read<T*>(L,1)->*_sfunc)(lua_tinker::read<V&>(L, 3));	}
//	};
//	template<typename T, typename BASE, typename VAR>
//	void class_p(lua_State* L, const char* name, VAR (BASE::*gfunc)(), void (BASE::*sfunc)(VAR& v)) 
//	{ 
//		lua_tinker::push_meta(L, lua_tinker::class_name<T>::name());
//		if(lua_istable(L, -1))
//		{
//			lua_pushstring(L, name);
//			new(lua_newuserdata(L,sizeof(p_var<BASE,VAR>))) p_var<BASE,VAR>(gfunc, sfunc);
//			lua_rawset(L, -3);
//		}
//		lua_pop(L, 1);
//	}
//
//void a(){
//	class_p<temp>(0, "rgba", &temp::get, &temp::set);
//}
...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	va_list args;
	va_start(args, fmt);
	vsnprintf(text, 10230, fmt, args);
	va_end(args);
	LogInfo("error->[%s] in file(%s)", text, GetCurrentLuaFile());
//
...
BOOL	lua_tinker::g_luaTnikerCallError = false;
