...
void	binary_fprintf(FILE* pFile, const char* szFmt,...)
{
	static char szText[10240];
	int w	= _vsnprintf(szText, 10240, szFmt, (va_list)(&szFmt+1));
	fwrite(szText, w, 1, pFile);
}
void	binary_fputs(const char* text, FILE* pFile)
{
...
TiXmlNode* TiXmlNode::FirstChild( const char * value ) const
{
	...
	{
		if ( stricmp(node->SValue().c_str(), TIXML_STRING( value ).c_str()) == 0 )
			return node;
	}
	return 0;
}
...
void TiXmlElement::Print( FILE* cfile, int depth ) const
{
	...
	for ( i=0; i<depth; i++ )
	{
		binary_fprintf( cfile, "    " );
	}

	binary_fprintf( cfile, "<%s", value.c_str() );
...
bool TiXmlDocument::SaveFile( const char* szHeadText ) const
{
...
bool TiXmlDocument::LoadFromMemory(const char* szText)
{
...
bool TiXmlDocument::SaveFile( const char * filename , const char* szHeadText) const
{
...
void TiXmlAttribute::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	if (value.find ('\"') == TIXML_STRING::npos)
		binary_fprintf (cfile, "%s=\"%s\"", n.c_str(), v.c_str() );
	else
		binary_fprintf (cfile, "%s='%s'", n.c_str(), v.c_str() );
}
...
void TiXmlText::Print( FILE* cfile, int /*depth*/ ) const
{
	...
	binary_fprintf( cfile, "%s", buffer.c_str() );
}
...
{
	binary_fprintf (cfile, "<?xml ");

	if ( !version.empty() )
		binary_fprintf (cfile, "version=\"%s\" ", version.c_str ());
	if ( !encoding.empty() )
		binary_fprintf (cfile, "encoding=\"%s\" ", encoding.c_str ());
	if ( !standalone.empty() )
		binary_fprintf (cfile, "standalone=\"%s\" ", standalone.c_str ());
	binary_fprintf (cfile, "?>");
}
...
void			TiXmlAttributeSet::Enum(std::vector<CXmlValueA>& rets)
{
	TiXmlAttribute* node;
	...
	{
		CXmlValueA attr;
		attr.name	=	node->name.c_str();
		attr.value	=	node->value.c_str();
		rets.push_back(attr);
	}
}
