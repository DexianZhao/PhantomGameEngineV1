...
//#endif
	...
	initState("states", (unsigned int*)stateGUID, sizeof(stateGUID));
	SetNextState(new MyGameState());
	g_manager.StopAllMusic();
}
VOID			CGameManager::OnCreate()
{
	//initState("states", (unsigned int*)stateGUID);
	//
...
#ifndef EDITOR_OS
	char code[512];
	sprintf(code, "function oninit()\ngamemgr:create(1);\nend\n");
	g_luamanager.loadAndExecute(code, szInFile);
#endif
}
