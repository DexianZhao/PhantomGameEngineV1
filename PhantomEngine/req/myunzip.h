...
#ifndef _zip_H
struct HZIP__{int unused;}; typedef struct HZIP__ * HZIP;
#endif
// An HZIP identifies a zip file that has been opened

typedef	FILE*	WINHANDLE;
typedef DWORD	ZRESULT;
// return codes from any of the zip functions. Listed later.
#define	MaxZipPath		256

enum ZipAttrib
{
	ZipAttrib_Directory	=	0x01,
	ZipAttrib_Archive	=	0x02,
	ZipAttrib_Hidden	=	0x04,
	ZipAttrib_ReadOnly	=	0x08,
	ZipAttrib_System	=	0x10,
};

typedef struct
{
	int index;                 // index of this file within the zip
	char name[MaxZipPath];      // filename within the zip
...
{
public:
	virtual	VOID					Append(const char* ptr, int length)	=	0;
	virtual	VOID					BeginWrite()	=	0;
};

HZIP OpenZip(const char *fn, const char *password);
HZIP OpenZip(void *z,unsigned int len, const char *password);
HZIP OpenZipHandle(WINHANDLE h, const char *password);
...
// Note: the item might be a directory (ze.attr & ZipAttrib_Directory)
...
ZRESULT UnzipItem(HZIP hz, int index, IUnzipFile* pFile);
ZRESULT UnzipItemHandle(HZIP hz, int index, WINHANDLE h);
